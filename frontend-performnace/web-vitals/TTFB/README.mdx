...
#### 以下幾項數值的總和，即 TTFB 所花費的時間：
  1. `Redirect 的時間`
  2. `service worker 啟動時間`
  3. `DNS 解析時間`
  4. `TCP 連線時間(三次握手的時間)`
  5. `接收到 first byte 的時間`

  TTFB 偏低越好，表示 browser 能越快收到 response 並開始做事

  在 `PR75` 內，在 `0.8s` 內視為 good score
...
3. 多透過 `stream` 將資源串流給 browser.
    ...
    >
    > 通常會用在大檔案傳輸，如果一個檔案 1GB，我們不可能等到全部傳輸完才開始處理，
    ...
    >
    > 可以參考[這邊](https://juejin.cn/post/6844904046436843527)的實作

    ---

4. 適當使用 `service worker`.
...
## Refs
  ...
  1. [web.dev](https://web.dev/articles/ttfb)

  2. [optimize TTFB](https://web.dev/articles/optimize-ttfb)

  3. [nodejs stream pj](https://pjchender.dev/nodejs/node-streams-and-pipes)

  4. [NodeJS official stream doc](https://nodejs.org/api/stream.html)

  5. [MDN Browser stream doc](https://developer.mozilla.org/en-US/docs/Web/API/Streams_API)

  這邊剛好本來就有三個關於 browser stream 的文章，有興趣可以參考，

  6. [全面理解 js stream api](https://juejin.cn/post/6992007156320960542)

  7. [精讀《web streams》](https://zhuanlan.zhihu.com/p/425322116)

  8. [stream api 解析](https://blog.csdn.net/weixin_44691608/article/details/121557249)
