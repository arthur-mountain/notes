...
如果專案中有輕量級標籤，默認情況下 `git describe` 可能不會考慮它們。使用 `--tags` 可以包含所有標籤。
...
在大型專案中，這有助於避免因分支導致的複雜歷史，提供更線性的描述。
...
假設已經在某個穩定版本上打了標籤，比如 `v1.0.0`，然後繼續進行了一些提交。在沒有打新標籤之前，使用 `git describe` 可以自動生成類似 `v1.0.0-3-gd5f7c32` 的版本號。
