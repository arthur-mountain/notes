...
3. **網路層(第三層) - IP 包(Packet)**
   ...
   - **IP header**:
     ...
     - **頭部長度(IHL, Internet Header Length)**:
       ...
       - 1. IPv4 Header 的基本結構和「頭部長度」字段

         IPv4 header 中的前 20 位元組(不含選項字段)是固定長度的，但選項字段是可選的，且長度不定，

         因此 IPv4 header 需要一個「頭部長度」字段來指示整個 header 的長度：

         - **IHL 字段的含義**：用來表示 IPv4 header 的總長度(包括選項字段)，單位是 4 位元組(32 bits)。

           - IHL 的值最小為 5(即 20 位元組)，這表示沒有選項字段的基本 IPv4 header。

           - 如果 IHL 的值大於 5，則表明有選項字段，並且解析器可以計算出選項字段的具體長度。

       - 2. 如何解析選項字段的長度

         解析器在讀取到 IHL 字段後，可以計算出整個 IPv4 header 的長度，因此能夠確定選項字段的開始和結束位置。

         例如:

         - **當 IHL = 5 時**：IPv4 header 長度為 20 位元組，沒有選項字段。

         - **當 IHL = 6 時**：IPv4 header 長度為 24 位元組，表示有 4 位元組的選項字段。

         - **當 IHL = 7 時**：IPv4 header 長度為 28 位元組，表示有 8 位元組的選項字段。

         解析器根據 IHL 的值，將多出來的部分當作選項字段進行解析。範例:

         假設 IHL = 7(表示 header 總長度為 28 位元組)：

         - 基本 header 長度為 20 位元組。(IPv4 header 中前 20 位元組(不含選項字段)是固定長度)

         - 選項字段長度為 `28 - 20 = 8 位元組`。

         - 解析器會解析基本 header 後，繼續解析 8 位元組的選項字段。
